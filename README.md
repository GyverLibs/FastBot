[![Foo](https://img.shields.io/badge/Version-2.10-brightgreen.svg?style=flat-square)](#versions)
...
// ответить на callback текстом (text) и режимом (alert): true - предупреждение, false - подсказка
...
- Внутри этой функции можно пользоваться переданной переменной `сообщение`, которая имеет тип `FB_msg` (структура) и содержит в себе:
    ...
    - `uint32_t unix` - время сообщения в формате unix
