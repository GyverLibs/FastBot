...
- Непосредственно в отправляющую функцию, у них у всех предусмотрен такой вариант (см. документацию выше)
```cpp
bot.sendMessage("Hello!", "123456");            // в один чат
bot.sendMessage("Hello!", "123456,7891011");    // в два чата
```
- Установить ID через `setChatID()` и все отправления пойдут в указанный чат/чаты
```cpp
...
## Белый список
В библиотеке реализован механизм белого списка: можно указать в `setChatID()` ID чата (или несколько через запятую), сообщения из которого будут приниматься. 
